...
impl WsReqUnsubscribe {
  pub fn new(guid: String, opcode: OpcodeEnum, token: String, ) -> WsReqUnsubscribe {
    WsReqUnsubscribe {
      guid: guid,
      opcode: opcode,
      token: token
  ...
  pub fn with_guid(mut self, guid: String) -> WsReqUnsubscribe {
    self.guid = guid;
    self
  }
  ///Уникальный идентификатор подписки, от которой отписываемся.
  ...
  pub fn with_opcode(mut self, opcode: OpcodeEnum) -> WsReqUnsubscribe {
    self.opcode = opcode;
    self
  }
  ...
  pub fn with_token(mut self, token: String) -> WsReqUnsubscribe {
    self.token = token;
    self
  }
  ///JWT токен для авторизации
